...
/*   Updated: 2019/04/30 17:52:20 by malluin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_info_header(t_image *image, char *str)
{
	...
	if (!(image->img_str = (unsigned char *)malloc(sizeof(unsigned char) *
...
t_image	*load_tga(char *path)
{
	...
	while ((ret = read(fd, str, PARSE_BUFF_SIZE)) != 0)
	{
		...
		while (i < ret)
		{
			image->img_str[idx] = str[i + 3];
			image->img_str[idx + 1] = str[i + 2];
			image->img_str[idx + 2] = str[i + 1];
			image->img_str[idx + 3] = str[i];
			// printf("B%3u G%3u R%3u A:%u ", image->img_str[i + 3],
			// image->img_str[i + 2], image->img_str[i + 1], image->img_str[i]);
			i += image->bits_color / 8;
			idx += image->bits_color / 8;
			// printf("B%3d G%3d R%3d A:%d \n", image->img_str[0 + 3],
			// image->img_str[0 + 2], image->img_str[0 + 1], image->img_str[0]);
			// if (i % (image->w * 4) == 0)
			// 	printf(" |%d|\n", i);
		}
	}
